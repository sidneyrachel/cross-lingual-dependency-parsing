...
class BiaffineEdgeScorer(nn.Module):
    def __init__(self, rnn_size, mlp_size, rel_size):
        super().__init__()
        ...
        self.rel_head_mlp = nn.Sequential(nn.Linear(rnn_size, mlp_size), mlp_activation)
        self.rel_dep_mlp = nn.Sequential(nn.Linear(rnn_size, mlp_size), mlp_activation)
